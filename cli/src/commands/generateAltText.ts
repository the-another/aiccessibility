        ...
        .action(async options => {
            try {
                ...
                if (!apiKey) {
                    console.error(
                        chalk.red(
                            'Error: OpenAI API key is required. Provide it with --api-key or set OPENAI_API_KEY environment variable.',
                        ),
                    );
                    process.exit(1);
                }
                ...
                if (options.file && options.base64) {
                    console.error(
                        chalk.red('Error: Please provide either a file or base64 data, not both.'),
                    );
                    process.exit(1);
                }
                ...
                if (!options.file && !options.base64) {
                    console.error(
                        chalk.red(
                            'Error: Please provide either a file path (--file) or base64 data (--base64).',
                        ),
                    );
                    process.exit(1);
                }
                ...
                if (options.file) {
                    if (!isValidImageFile(options.file)) {
                        console.error(
                            chalk.red(`Error: Invalid or non-existent image file: ${options.file}`),
                        );
                        process.exit(1);
                    }
