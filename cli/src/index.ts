...
program
    .command('detect-a11y')
    .description(
        'Tool for detecting a11y issues in a web page and trying to provide a solution for some of them.',
    )
    .option(
        '--threshold <number>',
        'Threshold of failures, that are tolerated for the numer of errors on the parsed page.',
        '0',
    )
    .option(
        '--include-tasks [tasks...]',
        'List of tasks to run. (all if none is selected/excluded)',
        Tasks.allAvailableTasks(),
    )
    .option(
        '--exclude-tasks [tasks...]',
        'List of tasks to exclude. (none if none is selected/included)',
        [],
    )
